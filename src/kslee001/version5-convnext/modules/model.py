...
from .convnext import ConvNeXt, ConvNeXtExpert
...
class A2IModelBase(tf.keras.Model):
    def __init__(self, configs):
        ...
        self.feature_extractor = ConvNeXt(
            blocks=self.configs.blocks,
            input_shape=(*self.configs.image_size,3),
            seed=self.configs.seed,
        ...
        self.atel_classifier = ConvNeXtExpert(    
        ...
        self.card_classifier = ConvNeXtExpert(    
            input_shape=self.feature_extractor.output.shape[1:],
            num_classes=3,
            conv_filters=self.configs.conv_filters,
            name='card',
        ...
        # [0,1] binary classification
        self.cons_classifier = ConvNeXtExpert(    
            input_shape=self.feature_extractor.output.shape[1:],
            num_classes=1,
            conv_filters=self.configs.conv_filters,
            name='cons',
        ...
        # [0, 1] binary classification 
        self.edem_classifier = ConvNeXtExpert(    
            input_shape=self.feature_extractor.output.shape[1:],
            num_classes=1,
            conv_filters=self.configs.conv_filters,
            name='edem',
        ...
        # [0,1,2] multiclass classification 
        self.plef_classifier = ConvNeXtExpert(    
            input_shape=self.feature_extractor.output.shape[1:],
            num_classes=3,
            conv_filters=self.configs.conv_filters,
            name='plef',
    ...
    def train_step(self, data):
        ...
        atel_gt = y[:, 0]
        atel_gt = tf.where(atel_gt==-1, 1.0, atel_gt) # float values needed !

        # card : multi (replace -1 with 2)
        card_gt = y[:, 1]
        card_gt = tf.where(card_gt==-1, 2.0, card_gt) 
        card_gt = tf.cast(card_gt, dtype=tf.int32) # onehot : integer needed
        card_gt = card_gt = tf.one_hot(card_gt, depth=3)

        # cons : ignore
        cons_gt = y[:, 2]
        cons_indices = tf.where(cons_gt != -1) # ignore -1 rows
        cons_indices = tf.reshape(cons_indices, [-1])
        cons_gt = tf.gather(cons_gt, cons_indices)

        # edem : ones
        edem_gt = y[:, 3]
        edem_gt = tf.where(edem_gt==-1, 1.0, edem_gt)

        # plef : multi
        plef_gt = y[:, 4]
        plef_gt = tf.where(plef_gt==-1, 2.0, plef_gt) 
    ...
    def initialize(self):
        self((tf.zeros((1, *self.configs.image_size, 3)), tf.zeros((1, 2))))
        self.augmentation(tf.zeros((1, *self.configs.image_size, 3)))