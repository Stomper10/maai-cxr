...
def transition_block(x, reduction, name, seed=1005, reg=0.):
    ...
    x = layers.BatchNormalization(
        axis=bn_axis, epsilon=1e-6, name=name + "_bn"
    )(x)
...
def conv_block(x, growth_rate, name, seed=1005, reg=0.):
    ...
    x1 = layers.BatchNormalization(
        axis=bn_axis, epsilon=1e-6, name=name + "_0_bn"
    )(x)
    ...
    x1 = layers.BatchNormalization(
        axis=bn_axis, epsilon=1e-6, name=name + "_1_bn"
    )(x1)
...
def ConvNeXt(
    ...
    x = layers.BatchNormalization(
        axis=bn_axis, epsilon=1e-6, name="stem/bn"
    )(x)
    ...
    x = dense_block(x, blocks[-1], name=f"block{len(blocks)}",
        seed=seed, reg=reg)
    x = layers.BatchNormalization(axis=bn_axis, epsilon=1e-6, name="bn")(x)
    x = layers.Activation("relu", name="relu")(x)

    # Create model.
    model = training.Model(img_input, x, name="convnext")
