...
if __name__ == '__main__':
	...
	# wrap sequences in tf.data.Dataset objects
	...
	def aggregate_gradients(grads):
		return [tf.reduce_sum(grad, axis=0) for grad in grads]


	@tf.function
	def apply_gradients(grads):
		strategy.run(lambda: model.optimizer.apply_gradients(zip(
	...
	def train_one_epoch(dataset):
		...
		for batch in dataset:
			...
			grads = aggregate_gradients(per_replica_gradients)
...
# Define a function to evaluate the model on the validation dataset
	def evaluate(dataset):
		...
		for inputs in dataset:
			# Run the validation step on each batch in parallel across all GPUs
			per_replica_losses, per_replica_accs = strategy.run(validation_step, args=(inputs,))
			# Aggregate the losses and accuracies across all GPUs
			loss = strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_losses, axis=None)
	...
	# Define a function to run the validation step on each batch of the validation dataset
	@tf.function
	def validation_step(inputs):
		...
		acc = metrics(labels, predictions)
	...
	for epoch in range(0, configs.num_epochs):
		...
		# train
		train_loss = train_one_epoch(train_dist_dataset)
		# validation
		valid_loss, valid_acc = evaluate(valid_dist_dataset)
		
		print(f"Epoch {epoch+1}/{configs.num_epochs}, Train Loss: {train_loss:.4f}, Val Loss: {valid_loss:.4f}, Val Acc: {valid_acc:.4f}")
