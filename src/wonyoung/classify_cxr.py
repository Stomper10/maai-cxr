...
def main():
  ...
  inference_time_list, labels_and_output = [], []
  count = 0
  for key in labels_dict:
    ...
    labels_and_output.append(labels_dict[key])
    
  print(f"Avg. {sum(inference_time_list) / len(labels_dict):.1f}ms per image.")
  
  labels = np.array([item[0] for item in labels_and_output])
  output = np.array([item[1] for item in labels_and_output])
